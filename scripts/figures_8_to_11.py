"""
Generate Figures 8, 9, 10, and 11.

Usage: python figures_8_to_11.py
"""
...
def plot_borderline_example(csv, cell_num=34, group="6-OHDA_mice_hsyn_ChR2_in_GPe"):
    types_abbrev = {
        "complete inhibition": "CI",
        "partial inhibition": "PI",
        "adapting inhibition": "AI",
        "no effect": "NE",
        "excitation": "EX",
        "biphasic IE": "BPIE",
        "biphasic EI": "BPEI",
    ...
    # Iterate through trials and gather data
    baselines = []
    stimuli = []
    binsize = 0.25
    all_fr = []
    bins = np.arange(-10, 10 + binsize, binsize)
    for trial in range(1, neuron.trials + 1):
        ...
        trial_spikes = (
            neuron.spikes[
                (neuron.spikes >= light_on[trial - 1] - 10)
                & (neuron.spikes < light_on[trial - 1] + 10)
            ]
            - light_on[trial - 1]
        )
    ...
    # Plot average firing rate over trials
    ...
    if all_average.shape[0] > 0:
        ...
        axes[1].fill_between(
            plot_bins[bins[:-1] <= 0],
            mean_all_average[bins[:-1] <= 0] + sem[bins[:-1] <= 0],
            mean_all_average[bins[:-1] <= 0] - sem[bins[:-1] <= 0],
            color="black",
            edgecolor="black",
            alpha=0.5,
    ...
    for trial in range(1, neuron.trials + 1):
        ...
        for k in range(len(psth)):
            psth[k] += len(
                combo[(combo >= psth_edges[k]) & (combo < psth_edges[k + 1])]
            )
    ...
    axes[0].add_patch(patches.Rectangle((0, neuron.trials + 0.2), 10, 0.2, color="r"))
    for i in ["left", "right", "top", "bottom"]:
        if i != "left" and i != "bottom":
    ...
    # Plot average SDF and shade excited areas
    avgsdf = np.zeros(
    ...
    for bi in range(len(bin_edges) - 1):
        ...
        axes[3].vlines(
            t[st], 0, avgsdf[st], linestyle="dotted", color="gray", alpha=0.5
        )
        if avg_bins[bi] == 2:
            st, ed = (
                np.where(t <= bin_edges[bi])[0][-1],
            ...
            axes[3].fill_between(
                t[st : ed + 1], avgsdf[st : ed + 1], color="blue", alpha=0.5
            )
    axes[3].vlines(t[ed], 0, avgsdf[ed], linestyle="dotted", color="gray", alpha=0.5)
    axes[3].hlines(
        2 * neuron.excitation_threshold, 0, 10, linestyle="dashed", color="gray"
    )
    ...
    avgisif = np.zeros(
        len(
            np.loadtxt(
                f"{neuron.cell_dir}/trial_{trial:02d}/stimulus_data/{'isif' if neuron.isif_inhibition else 'sdf'}.txt"
            )
        )
    )
    for trial in range(1, neuron.trials + 1):
    ...
    # Plot average ISIF and shade inhibited areas
    ...
    for bi in range(len(bin_edges) - 1):
        ...
        axes[4].vlines(
            t[st], 0, avgisif[st], linestyle="dotted", color="gray", alpha=0.5
        )
        if avg_bins[bi] == 1:
            axes[4].fill_between(
                t[st : ed + 1], avgisif[st : ed + 1], color="blue", alpha=0.5
            )
    axes[4].vlines(t[ed], 0, avgisif[ed], linestyle="dotted", color="gray", alpha=0.5)
    axes[4].hlines(
        2 * neuron.inhibition_threshold, 0, 10, linestyle="dashed", color="gray"
    )
...
# Figure 8
plot_borderline_example(csv, cell_num=85, group="6-OHDA_mice_hsyn-ChR2_in_GPe")

# Figure 9
plot_borderline_example(csv, cell_num=82, group="6-OHDA_mice_hsyn-ChR2_in_GPe")

# Figure 10
plot_borderline_example(csv, cell_num=79, group="6-OHDA_mice_hsyn-ChR2_in_GPe")

# Figure 11
plot_borderline_example(csv, cell_num=16, group="6-OHDA_mice_hsyn-ChR2_in_GPe")
