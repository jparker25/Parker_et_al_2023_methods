...
# Designate directories for example
...
axes[0].set_yticks([])
rect_sdf = patches.Rectangle(
    (2.25, 0.9), 0.5, 0.2, fill=False, color="gray", linewidth=2
)
axes[0].add_patch(rect_sdf)
rect_isi = patches.Rectangle(
    (7.5, 0.9), 0.5, 0.2, fill=False, color="gray", linewidth=2
)
axes[0].add_patch(rect_isi)
axes[0].set_ylim([0.8, 1.2])
axes[0].set_xlabel("Time (s)")

# Find appropriate sections of data
sdfA = [2.375, 0.9]
isifA = [7.625, 0.9]
sdfB = [2.5, 15]
isifB = [7.75, 0.2]
...
# Add arrows
arrow_sdf = patches.ConnectionPatch(
    sdfA,
    sdfB,
...
arrow_isif = patches.ConnectionPatch(
    isifA,
    isifB,
    coordsA=axes[0].transData,
    coordsB=axes[4].transData,
    color="gray",
    arrowstyle="-|>",
    mutation_scale=20,
    linewidth=2,
...
# Plot SDFs snippets and zoom in
...
linterp_examp = patches.Rectangle(
    (time[tpt - 125], 0),
    250 / 1000,
    0.2,
    fill=False,
...
axes[5].annotate(
    "",
    xy=(time[tpt], isif[tpt]),
    xytext=(time[tpt], -0.025),
...
rect_isi = patches.Rectangle(
    (7.5, 0.015), 0.5, 0.1, fill=False, color="gray", linewidth=3
)
...
os.system(f"open ../figures/figure_1.pdf")
